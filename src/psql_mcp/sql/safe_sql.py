...
class SafeSqlDriver(SqlDriver):
    ...
    def __init__(
        ...
        Args:
            ...
            allowed_schemas: If set, reject queries that reference schemas outside this set
        """
    ...
    def _collect_schema_references(self, node: Node, schemas: set[str]) -> None:
        if isinstance(node, RangeVar) and node.schemaname:
            schemas.add(node.schemaname)
        ...
        for attr_name in node.__slots__:
            ...
            if isinstance(attr, list):
                for item in attr:
                    if isinstance(item, Node):
                        self._collect_schema_references(item, schemas)
            elif isinstance(attr, tuple):
                for item in attr:
                    if isinstance(item, Node):
                        self._collect_schema_references(item, schemas)
            elif isinstance(attr, Node):
                self._collect_schema_references(attr, schemas)

    def _validate_schema_allowlist(self, parsed: list) -> None:
        if not self.allowed_schemas:
            return

        referenced: set[str] = set()
        for stmt in parsed:
            self._collect_schema_references(stmt, referenced)

        disallowed = {
            s
            for s in referenced
            if s not in self.allowed_schemas
            and s not in SYSTEM_SCHEMAS
            and not s.startswith("pg_")
        }
        if disallowed:
            raise ValueError(
                f"Query references schema(s) not in ALLOWED_SCHEMAS: {', '.join(sorted(disallowed))}"
            )
    ...
    def _validate(self, query: str) -> None:
        ...
        try:
            ...
            self._validate_schema_allowlist(parsed)
